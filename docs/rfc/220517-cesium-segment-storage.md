...
(1Hz, 5Hz, 10Hz, etc.) \
**Samples/Second** - A basic measure of write throughput. The size of a regular sample should be assumed as 8 bytes (i.e. a
float64 value) unless otherwise specified, whereas an irregular sample is assumed to contain an additional 64 bit timestamp.
...
Writing a database storage engine is quite an undertaking, taking years and many development cycles, so we'd ideally use
...
own
[benchmarks](https://cockroachdb.github.io/pebble/) show a maximum write throughput of 60,000 values per second, far
below
Arya Core's throughput of 6 million values per second. An elastic throughput in the range several hundreds of millions
of
...
it [tstorage](https://github.com/nakabonne/tstorage)
which tailored towards irregular time-series data. It's benchmarks shows a maximum write throughput of about 3,000,000
samples per second.

Delta is unique in that almost all of its uses involve storing regular time-series data. This is a huge advantage in
terms of database simplicity and performance. `tstorage` doesn't take advantage of data regularity, and is missing out
...
the [embed](https://pkg.go.dev/embed) . This package allows for in-application (embedded) bootstrapping, but API calls 
to the key-value interfaces must still be done using a client over a network. 

etcd uses RAFT to achieve consensus, and replicates writes to all nodes in the cluster. This means that write amplification
scales along with the number of cluster members. This is ok for small clusters, but quickly becomes a problem for larger
clusters (the authors of etcd advise against running a cluster with greater than seven nodes).

etcd's write amplification over the network is also problematic for large quantities of data. It's unreasonable to expect
write throughputs in the tens of millions of samples/s for a cluster of seven nodes, even over a very performant
network.
...
atomic clocks. It focuses on simplicity through two means:

1. Restricting the attributes of the time-series data that can be stored (see [Restrictions to the Time-Series Use Case](#restrictions-on-time-series)). 
2. Restricting the available query patterns to a simple start-end lookup (while still allowing future implementations 
...
Delta is designed to work with data acquisition hardware, and as such, must be optimized for time-series data that arrives
...
A channel's **sample rate** must be predefined. This is by far the largest restriction and optimization that Cesium makes. 
...
between samples are not validated or stored. Even the most precise sensors and devices have minor irregularities in their
sample rates (+/- a few nanoseconds in the case of most data acquisition computers (DAQ)). Cesium leaves it to the caller to 
...
to monitor the number of requests to a particular API endpoint. The web server pushes this data to a Cesium backed monitoring
service at intervals of 5 seconds +/- 1 second. Cesium would assume these values are written to the channel at even, five second intervals 
e.g. 0s, 5s, 10s, 15s as opposed to 0s, 6s, 9s, 15s, etc. The DevOps engineer probably doesn't care about the exact 
